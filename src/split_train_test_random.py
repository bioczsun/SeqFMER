...
def generate_random_dna_seq(nucleotides,probs,seq_len=600):
    return ''.join([random.choices(nucleotides, probs)[0] for _ in range(seq_len)])
...
def get_train_test_data(peaks_file,nopeaks_file,fasta,outpath,seq_len=256,slide=1024):
    ...
    for i in train_data_pos:
        ...
        sequence_neg = generate_random_dna_seq(nucleotides,probs,seq_len)
    ...
    data_outpath = "%s/train_test_random_%s.npz"%(outpath,seq_len)
