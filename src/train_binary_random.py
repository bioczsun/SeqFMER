...
def custom_collate_fn(batch):
    ...
    for item in batch:
        ...
        sequence = utils.onehot_seq(item[0])
...
model_save_path = "%s_%s_%s_%s_random"%(args.model,args.seed,args.seqlen,args.activate)
...
def eval(model,test_dataloader,save_path,model_name,criterion,device):
    ...
    with torch.no_grad():
        ...
        df.to_csv("%s/eval_result_%s_random.csv"%(save_path,model_name))
        df1 = pd.DataFrame({"precision":precision,"recall":recall})
        df1.to_csv("%s/eval_pr_result_%s_random.csv"%(save_path,model_name))
        f = open("%s/classification_report_%s_random.txt"%(save_path,model_name),"w")
