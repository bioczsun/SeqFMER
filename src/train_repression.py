...
params.add_argument("--activate",help="activate function",required=True)
...
for epoch in range(num_epochs):
    ...
    for index,(sequences, coverage) in enumerate(train_loader):
        ...
        # print(outputs,coverage)
        # Compute the loss
        print(torch.log2(outputs+1), torch.log2(coverage+1))
        loss = criterion(torch.log2(outputs+1), torch.log2(coverage+1))# + 2 * pearson_loss(torch.log2(outputs), torch.log2(coverage))
