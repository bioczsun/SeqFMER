...
sequence_length = 600  # DNA序列的长度

# 初始化模型
model = vqvae.VQVAE(input_dim, embedding_dim, num_embeddings, commitment_cost,sequence_length)
model.to(device)
...
train_loader = DataLoader(train_dataset, batch_size=int(args.batch),collate_fn=custom_collate_fn,num_workers=8,sampler=sampler) 
...
def one_hot_to_dna(one_hot_tensor):
    # 定义索引到DNA碱基对的映射
    ...
    # 将索引转换为相应的DNA碱基对字符
    dna_sequence = ''.join([index_to_base[i] for i in indices])
...
def train(model,train_dataloader,test_dataloader,optimizer,criterion,epochs,early_stopping,scheduler,save_path,model_name,device):
    ...
    for epoch in range(epochs):
        ...
        for index,(data,label) in enumerate(train_dataloader):
            ...
            if (index + 1) % 100 == 0:
                print('Epoch [{}/{}], Step [{}/{}], Loss: {:.4f}, auc: {:.4f}'
                        .format(epoch + 1, epochs, index + 1, total_step, loss.item(), 1))
                print("original seq:")
                print(one_hot_to_dna(data[0].detach().cpu()))
                indices = torch.argmax(seq[0].transpose(0,1), dim=1)
                one_hot = F.one_hot(indices, num_classes=seq[0].transpose(0,1).size(1)).float()
                print("reconstruct seq:")
                print(one_hot_to_dna(one_hot.detach().cpu()))
        ...
        #     break #跳出迭代，结束训练
        torch.save(model.state_dict(), "%s/%s_%s_best_network.pth"%(save_path,model_name,epoch))
